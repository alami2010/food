...
export class AppComponent {
  ...
  navigationInterceptor(event: RouterEvent): void {
    ...
    if (event instanceof NavigationEnd) {
      ...
      this.titleService.setTitle(data && data[0] ? this.util.translate(data[0]) + ' | Foodies' :
        this.util.translate('Home') + ' | Foodies');
    }
  ...
  initializeApp() {
    ...
    this.api.get('v1/settings/getDefault').then((data: any) => {
      console.log(data);
      if (data && data.status && data.status == 200 && data.data && data.data.general && data.data.settings && data.data.support) {
        ...
        this.cart.freeDeliveryCharge = parseFloat(general.free_delivery);
        // this.cart.flatTax = parseInt(general.tax);
        // this.cart.freeShipping = parseFloat(general.free_delivery);
        this.util.allowDistance = general.allowDistance;
        this.util.facebook_link = general.facebook_url;
        this.util.instagram_link = general.instagram;
        this.util.twitter_link = general.twitter;
        this.util.google_playstore = general.google_playstore;
        this.util.apple_appstore = general.apple_appstore;
        this.util.web_footer = general.web_footer;
    ...
    if (uid && uid !== null && uid !== 'null') {
      ...
      this.api.post_private('v1/profile/getByID', { id: localStorage.getItem('uid') }).then((data: any) => {
        console.log('*******************', data);
        if (data && data.status && data.status == 200 && data.data && data.data.type == 1) {
    ...
    if (this.userCode) {
      ...
      this.api.post('v1/otp/verifyOTP', param).then((data: any) => {
        ...
        if (data && data.status == 200) {
          ...
          this.util.start();
          this.api.post('v1/auth/create_account', registerParam).then((data: any) => {
            this.isLogin = false;
            this.util.stop();
            console.log(data);
            if (data && data.status == 200) {
      ...
      this.api.post_private('v1/referral/redeemReferral', body).then((data: any) => {
        ...
        if (data && data.status && data.status == 200) {
          ...
          if (info && info.who_received == 1) {
            msg = 'Congratulations your friend have received the' +
              this.util.currencySymbol +
              info.amount +
              ' ' +
              'on wallet';
          } else if (info && info.who_received == 2) {
            msg = 'Congratulations you have received the ' +
              this.util.currencySymbol +
              info.amount +
              ' ' +
              'on wallet';
          } else if (info && info.who_received == 3) {
            msg = 'Congratulations you & your friend have received the ' +
              this.util.currencySymbol +
              info.amount +
              ' ' +
              'on wallet';
